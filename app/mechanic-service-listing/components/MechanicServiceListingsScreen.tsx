...
function MechanicProfileDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-white w-full sm:w-[480px] max-h-full overflow-y-auto shadow-modal animate-slide-up sm:animate-none">
        <div className="sticky top-0 bg-white border-b border-brand-border px-5 py-4 flex items-center justify-between z-10">
          <h2 className="text-base font-bold text-brand-fg">Mexanik Profili</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              {
                label: "Durum",
                value: service.isAvailable ? "Mövcud" : "Məşğul",
                Icon: FaClock,
              ...
              >
                <stat.Icon
                  size={18}
                  className={`mx-auto mb-1.5 ${stat.color ?? "text-primary-DEFAULT"}`}
                />
                <p
                  className={`text-sm font-bold ${stat.color ?? "text-brand-fg"} tabular-nums`}
                >
                  {stat.value}
                </p>
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-3 p-3 bg-brand-bg rounded-xl border border-brand-border">
              <FaPhone size={16} className="text-primary-DEFAULT shrink-0" />
              <span className="text-sm font-mono text-brand-fg">
                {service.phone}
              </span>
            </div>
            <div className="flex items-center gap-3 p-3 bg-brand-bg rounded-xl border border-brand-border">
              ...
              />
              <span className="text-sm text-brand-fg">{service.location}</span>
            </div>
          </div>

          <div>
            <h4 className="text-sm font-bold text-brand-fg mb-2.5">
              İxtisaslar
            </h4>
            <div className="flex gap-2 flex-wrap">
              {service.specializations.map((spec) => (
                <span
                  key={`drawer-spec-${service.id}-${spec}`}
          ...
          <div>
            ...
            <div className="p-4 bg-brand-bg rounded-xl border border-brand-border">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h5 className="text-sm font-bold text-brand-fg">
                  {service.serviceName}
                </h5>
              </div>
              <p className="text-sm text-brand-muted-fg leading-relaxed mb-3">
                {service.description}
              </p>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-bold text-primary-DEFAULT tabular-nums">
                    {service.priceMin} – {service.priceMax} ₼
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {allServices
                  ...
                  .map((s) => (
                    <div
                      key={`other-svc-${s.id}`}
                      className="flex items-center justify-between p-3 bg-brand-bg rounded-xl border border-brand-border"
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <span className="text-sm font-medium text-brand-fg truncate">
                          {s.serviceName}
                        </span>
                      </div>
                      <span className="text-sm font-bold text-brand-fg tabular-nums shrink-0 ml-2">
        ...
        <div className="sticky bottom-0 bg-white border-t border-brand-border px-5 py-4">
          ...
          >
            <FaPhone size={15} />
            {tServices("contact")} — {service.phone}
          </button>
        </div>
      </div>
    </div>
